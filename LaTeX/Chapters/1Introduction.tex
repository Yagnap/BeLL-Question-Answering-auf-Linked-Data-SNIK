...
Es fasst\todo{fasst klingt hier komisch} Wissen aus drei Lehrbüchern, welches es in semantischer Weise modelliert und publiziert.
In der Ontologie (siehe \cref{sub:ontology}) sind Zusammenhänge und Verbindungen zwischen verschiedenen Informationen vorhanden, welche auf der öffentlich erreichbaren Visualisierung in Form eines Graphen, der diese aufzeigt, dargestellt sind.\todo{satz zu lang und überschneidet sich mit verschiedenen lösungen untesatz zu lang und überschneidet sich mit verschiedenen lösungen unten}
...
\begin{enumerate}
	\item das Verwenden eines \acs{rdf}-Browsers,\todo{po et al. (mit seite) zitieren} der \acs{rdf}-Informationen einzeln darstellt,
	\item den \ac{snik}-Graphen oder\todo{leser wissen hier nicht was das ist, ich würde sagen eine graphvisualisierung und po et al. zitieren}
...
Im Fall von SPARQL nimmt es den Medizininformatik-Studierenden wertvolle Zeit, da sie sich dort erst in die Syntax des Programms\todo{sparql ist eine abfragesprache und kein programm} und das Vokabular des Fachbereichs einarbeiten müssen.
