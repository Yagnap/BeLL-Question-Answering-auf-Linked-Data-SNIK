...
Wenn man mehrere Beziehungen abfragen will, kann man \emph{property paths} verwenden.
\todo{es geht auch zwischen mehr als zwei, also a|b|c|... }
\todo{das stimmt so nicht, die abfrage wird nicht mehrmals ausgeführt.}
\todo{zitiere mal und lies mal durch:  SPARQL 1.1 Query Language, Section 9: Property Paths. unter https://www.w3.org/TR/sparql11-query/#propertypaths . es gibt leider nur webquellen für die w3c standards.}
\todo{das pipe symbol | ist nur eines von vielen möglichkeiten, property paths zu bilden, siehe 9.1 Property Path Syntax. | steht für alternativer pfad. es gibt auch z.B. / für Sequenz, ^ für inverse, * für 0 oder mehrfache wiederholung usw.}
Bei diesen wird zwischen zwei Prädikate ein \texttt{|} geschrieben, \ac{sparql} führt die Abfrage dann mehrere Male, einmal für jedes Prädikat, aus.
...
\todo{hattet ihr reguläre ausdrücke in der Schule? Wenn nicht müsstest du das auch irgendwo erklären, was reguläre ausdrücke sind. also jetzt nicht so super ausführlich, da kann man ja bücher drüber schreiben, sondern nur die wesentlichsten sachen.}
Bei beiden dieser Methoden kann man auch reguläre Ausdrücke nutzen.
\todo{nee, filter benutzt reguläre ausdrücke aber nur wenn man FILTER regex(...) benutzt. aber sparql property paths benutzen die property path syntax, die sich aber teilweise an reguläre ausdrücke anlehnt, z.B. mit dem * operator}
Dies ist nützlich, um bei \ac{snik} gleich über alle drei Teilontologien Abfragen zu stellen.
\todo{wieso braucht man reguläre ausdrücke um mehrere teilontologien gleichzeitig abzufragen?}
...
Mit dem Charakter\todo{engl character deutsch zeichen} \texttt{*} wird auch \aurl{bb}{ChiefInformationOfficer} selbst ausgewählt.
