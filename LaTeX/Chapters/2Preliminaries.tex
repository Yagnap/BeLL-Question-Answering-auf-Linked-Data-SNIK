...
\todo{Die Arbeit ist ja auf Deutsch, aber die QA Sprache Englisch. Das würde ich daher noch deutlich machen an welcher Stelle die Sachen für alle Sprachen gelten und an welcher Stelle nur für Englisch. Das gilt für die ganze Arbeit.}
...
Da die Sprache, die für Question Answering verwendet wird, am ehesten und auch in dieser Arbeit Englisch ist,
...
\todo{bitte noch erklären, was eine Lexikalisierung ist.}
...
\todo{Achtung! Du darfst kein Buch über Deutsch zitieren wenn die Sprache Englisch ist!}
...
Es ist außerdem nicht nur ein verwendbares Artefakt wie das \ac{www}, sondern auch ein Forschungsfeld \citep{semanticwebreview}, das sehr stark von anderen Forschungsfeldern, wie etwa Deep Learning und künstlicher Intelligenz, abhängig ist.
\todo{Ist das Semantic Web wirklich sehr stark vom Deep Learning abhängig? Das ist ja schon eine starke Aussage, das bitte belegen.}
...
\todo{zu klein. wenn es mit normaler Größe nicht passt probiere es mal eine Größe größer}
\subsection{RDF}
...
\end{figure}
\todo{Quelle für Bild fehlt in Caption für Bild 2.1 Hierarchie}
...
\todo{Musterfrau oder Mustermann? Eines auswählen und konsistent verwenden bitte.}
...
\todo{Ich glaube die Zitierung ist an der falschen Stelle, das Beispiel ist doch von dir.}
...
\todo{Achtung overfull hboxes (Schrift ragt über den Rand, das darf nicht passieren)}
...
\todo{Achtung! Literale und Datatype Properties sind nicht das Selbe. Literale sind Zeichenketten und Datatype properties sind properties deren Wertebereich Literale sind.}
...
\todo{Der Definitionsbereich einer Property ist immer eine Klasse.}
...
%\todo{Ungenau. Ich würde hier eher sagen, dass die Anzahl der Spalten und damit die Arität (Stelligkeit) der Relationen bei relationalen Datenbanken wesentlich höher ist (je nach Art der Datenbank wohl 1000 und mehr, aber theoretisch könnte man vielleicht schon sagen unbegrenzt).}
...
\todo{wissen deine Leser, was reguläre Ausdrücke sind? Ansonsten kurz erklären.}
...
\end{lstlisting}
Da die Teilontologien untereinander mit \aurl{meta}{closeMatch} verbunden sind, wird diese Beziehung hier abgefragt.
...
Mit dem Zeichen \texttt{*} wird auch \aurl{bb}{ChiefInformationOfficer} selbst ausgewählt.
\todo{richtig aber kann noch genauer beschrieben werden: \texttt{*} steht für beliebige Wiederholung des Pfades inklusive 0-mal, daher also auch bb CIO selbst.}
...
\end{definition}
\todo{Achtung! Der Unterschied zwischen Question Answering und Semantic Question Answering ist nicht, dass nur bei Semantic Question Answering die Fragen in
natürlicher Sprache gestellt werden! Natürliche Sprache wird auch bei ganz normalem Question Answering benutzt aber nicht z.B. bei keyword search wie Google.
Das sieht man ja auch an deiner zitierten Quelle für die Definition von QA: Natural Language Question Answering.
Außerdem passt die Definition und Quelle nicht zu dem, was ich mit Semantic Question Answering meine.
Ich meine damit Question Answering auf RDF Daten, also Semantic im Sinne von Semantic Web.
Wenn dir das zu uneindeutig ist, nimmst du vielleicht besser den heute wohl gebräuchlicheren Ausdruck KBQA (knowledge base QA).
Oder du lässt es bei Semantic Question Answering, nimmst aber eine passendere Quelle.
Beispielsweise unsere Survey :-) \url{https://content.iospress.com/articles/semantic-web/sw247}
}
...
\todo{Achtung! Zeilen sind hier keine passende Maßeinheit! Man kann ja die gleiche SPARQL Abfrage in eine oder in viele Zeilen packen. Es geht eher darum, wieviele
Triple Patterns, Filter und so weiter drin sind. Außerdem hat eine Frage keine SPARQL Abfrage sondern die Antwort kann hoffentlich durch verschiedene SPARQL
...
\todo{bitte auch hier nicht von Zeilen schreiben}
...
Erst später werden für weitere Funktionen über Repräsentationen hinzugefügt.eep contextualized word representations\todo{fix}
...
\todo{Die Quelle ist unvollständig und kann so nicht benutzt werden! Im Literaturverzeichnis stehen nur Autoren, Titel und Jahr aber kein Journal oder Konferenzband! Guck mal, ob das vielleicht gar kein Paper ist.}
\todo{OpenAI ChaptGPT ist seit November 2022 in aller Munde. Super Chance für dich, die Leser mitzureißen! Ich würde das unbedingt erwähnen, entweder hier und/oder in der Einleitung oder Diskussion oder Ausblick. Und in mehr Detail beschreiben, was genau GPT ist und wie es sich von ChatGPT abhebt und was das mit Question Answering zu tun hat.}
...
\todo{Mach am besten mal ne Rechtschreibprüfung auf die ganze Arbeit.}
...
\todo{meinst du: Dadurch erreicht BERT in Benchmarks... ?}
...
\todo{Mir fehlt die Verknüpfung dieses Kapitels mit dem Rest der Arbeit. Bitte in den folgenden Kapiteln erklären, wo dort maschinelles Lernen, neuronale Netze, Deep Learning und so benutzt werden. Bei TeBaQa?}
...
\todo{Kann man das so schreiben? Für mich klingt besser: Sie basieren auf...}
...
\subsection[LSTMs und GRUs]{Long short-term memory und Gated Research Units}\label{sub:lstm}
