...
Bei der \ac{qald}-Challenge werden Question Answering-Systeme anhand eines Benchmark objektiv bewertet.
Diese fand schon mehrmals statt, die zur Zeit dieser Arbeit aktuelle ist \ac{qald}-9 \citep{qald9}
\footnote{\ac{qald}-9-Plus nahm die Fragen aus \ac{qald}-9 und übersetzte diese in acht verschiedene Sprachen (Deutsch, Französisch, Russisch, Ukrainisch, Belarussisch, Armenisch, Baschkirisch, Litauisch),
um Entwicklung im Bereich der Multilingualität von Systemen voranzutreiben, gilt aber nicht als offizielle Version der \ac{qald}-Challenge \citep{qald9plus}.},
