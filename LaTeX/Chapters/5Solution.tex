...
Statt etwa \texttt{\aurl{meta}{isResponsibleForEntityType}} stünde nun \texttt{\aurl{meta}{isResponsibleForRole} | \aurl{meta}{isResponsibleForFunction} | \aurl{meta}{isResponsibleForEntityType}} dort.
Praktisch für die Lokalisierung der Fehler war die Funktion, sich alle generierten Anfragen anzeigen zu lassen.
\todo{wir sind doch beim Training, warum steht hier Lokalisierung?}
Somit konnte erahnt werden, warum es das macht, was es macht.
\todo{Dieser ganze Abschnitt ist total verwirrend.
Jemand der kein Experte in dem Gebiet ist versteht so gar nichts.
An dieser Stelle fehlt der Hinweis, das QAnswer keine Property Paths beherrscht, damit geht auch der Sinn verlonre.
Lies ihn dir bitte mal im Ganzen durch und achte darauf, dass alle logischen Schlussfolgerungen auch stimmen.
Da fehlen Verbindungsstücke.}
Die ganze Abfrage sähe dann so aus:\todo{dann wann?}
\begin{lstlisting}[language=SPARQL]
  SELECT ?o
  WHERE {
...
Eine andere Möglichkeit, diese Frage mittels einer \ac{sparql}-Abfrage zu beantworten, ist mittels der \aurl{rdfs}{subPropertyOf}-Beziehung:
\begin{lstlisting}[language=SPARQL]
  SELECT ?o
  WHERE {
...
\end{lstlisting}
Dabei wichtig ist, dass hier \texttt{*} statt \texttt{+} verwendet wird, falls \aurl{meta}{isResponsibleFor} selbst genutzt wird,
und nicht nur die untergeordneten Properties.
\todo{wieso deshalb? worauf bezieht sich das?}
...
\todo{statt texttt bitte aurl nehmen für ressourcen mit prefix}
...
\todo{bitte erklären warum das gemacht wurde.}
...
\todo{meta + bb mergen}
\todo{transitive Subklassenbeziehungen materialisieren}
\todo{JS Error 405}
\todo{Textbuchfragen abfragen}
