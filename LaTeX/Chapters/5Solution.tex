...
GROUP_CONCAT(STR(?sl),"; ") as ?answers,
?p, ?o
FROM sniko:meta
FROM sniko:bb
{
...
In der eigentlichen Abfrage wird festgelegt, dass das Prädikat den Definitions- und Wertebereich \aurl{rdfs}{subClassOf} \aurl{meta}{top} haben muss, also keine Literale beinhalten soll.
Dann werden das Objekt sowie die Subjekte und deren Labels ausgewählt.
...
SELECT CONCAT(
    "What ", REPLACE(REPLACE(REPLACE(
          ?pl, "^is ([a-z]* [a-z]*)", CONCAT("is ", ?sl, " $1")
        ), "^([a-z]*e)s", CONCAT("is $1d by ", ?sl)
      ), ".* component", CONCAT("are components of ", ?sl)
    ), "?") as ?question,
GROUP_CONCAT(STR(?ol),"; ") as ?answers,
?s, ?p
FROM sniko:meta
FROM sniko:bb
{
...
Letztlich, die \ac{sparql}-Abfrage für die Fragen nach den Prädikaten.
Hierfür gibt es ein striktes Muster, was eine vergleichsweise einfache Abfrage ermöglicht:
\begin{lstlisting}[language=SPARQL]
# SPARQL-Abfrage für Fragen nach dem Prädikat
SELECT CONCAT("How are ", ?sl, " and ", ?ol, " related?") as ?question,
?pl as ?answers,
?s, ?o
FROM sniko:meta
FROM sniko:bb
...
GROUP BY ?s ?o ?sl ?ol
ORDER BY RAND()
\end{lstlisting}\todo{Virtuoso 22023 Error SR066: Unsupported case in CONVERT (incomplete RDF box -> VARCHAR)}

\todo{Anzahl der Fragen, die dadurch generiert werden? Und was damit gemacht wird (wie im Benchmark genutzt - alle oder nur die Hälfte oder so Training?)}
