...
Diese sind \texttt{4/3}, \texttt{6/1}, \texttt{8/4}, \texttt{8.4/1} und \texttt{9/2} sowie \texttt{9.3.4.2/3} bis \texttt{9.3.4.2/5} und \texttt{10/3}.
Somit verbleiben letztendlich 38 Fragen, welche im Folgenden in \ac{sparql}-Abfragen umgewandelt werden.
...
SELECT DISTINCT ?s1
WHERE
  { ?s1 rdfs:subClassOf bb:StrategicHISDirecting . }
