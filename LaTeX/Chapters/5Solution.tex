...
\todo{noch mit einbauen, dass Dr. Franziska Jahn bei der korrektur geholfen hat aber kein extra Satz, vielleicht: gebildet und durch Dr. Franziska Jahn korrigiert}
...
Diese Antworten sollen also Nutzer, die die jeweiligen Fragen fragen, erhalten:\todo{die beiden Sätze zu einem Zusammenfassen}
\todo{die von Franziska als falsch bewerteten Antworten könnte man z.B. durch fett gedruckt hervorheben. oder waren das sachen die wir auf unserer Seite korrigieren müssen? die antworten müssen ja alle richtig sein beim goldstandard}
\todo{nach frage 6/2 kommt 6/4. in der tabelle steht 6/3 aber noch drin. soll das so sein?}
...
\todo{einfachere -> einfache (überall)}
\todo{für die erstellung der fragen müssen viele fragen erstellt werden? was ist das für eine aussage?}
...
\todo{das präfix sniko hast du in dieser arbeit noch nicht definiert. es steht für http://www.snik.eu/ontology/. entweder du schreibst es in deine präfixtabelle oder als PREFIX oben in die sparql abfrage. gilt auch für andere stellen wo das vorkommt.}
\todo{die replaces würde ich gleich einrücken, habe ich gleich mal unten gemacht}
\todo{bei der SPARQL Abfrage habe ich dir doch geholfen, das muss man eventuell noch mit erwähnen, oder steht das woanders? also kein extra satz aber vielleicht kann man vom betreuer bereitgestellt oder unter mithilfe des betreuers irgendwo reinschieben}
\begin{lstlisting}[language=SPARQL]
...
Zuerst wird die Frage mithilfe der Zeichenketten-Funktionen generiert.\todo{was ist denn eine zeichenketten-funktion? meinst du concat? das steht für Konkatenation, die Verkettung von Zeichen oder Zeichenketten.}
...
\todo{auch hier bitte einrückung nicht syntaktisch sondern auf selbe höhe von den replace aufrufen, finde ich übersichtlicher}
\begin{lstlisting}[language=SPARQL]
...
ORDER BY RAND()
\end{lstlisting}
\todo{ich würde die kommas weglassen}
...
Die Auswahl eines oder mehrerer Kandidaten gestaltete sich aufgrund von nicht instand gehaltener Programme\todo{grammatik: aufgrund von Programmen...ähnliche}
...
bei gAnswer war die Vorbereitung der Daten sehr aufwändig und die gegebenen Werkzeuge kaputt.\todo{kaputt ist informell, kann man das präzisieren?}
DeepPavlov war am Anfang sehr vielversprechend, besonders aufgrund des modularen Aufbaus.
Jedoch gab es keine Möglichkeit, andere Daten als Wikidata zu verwenden, weshalb es für den Einsatz mit \ac{snik} wenig hilfreich ist.\todo{Präsenz: gab -> gibt}
\todo{füllwörter wie wenig vermeiden. wenig hilfreich -> z.B. nicht nutzbar}
\todo{asknow ist doch kein ort, daher passt wo meiner Meinung nach nicht}
...
\todo{sollte -> soll -> eigentlich ganz weglassen}
\todo{gab -> gibt}
...
\todo{quasi ist hier ein füllwort. wir...habe -> wir ... haben}
...
\todo{diese war darin? es wurden werde? was soll das heißen? wir konnten nicht alle Fehler beheben, da es zu Fehlern kam?}
...
\todo{warum phases und methods unterschiedlich formatieren?}
Dies wurde hier für \enquote{phases} und \emph{methods} bei \aurl{meta}{updates} sowie \enquote{tasks} und \aurl{meta}{functionComponent} gemacht.
...
\todo{wo das kapitel, dass das kapitel?}
...
Nach dem Training haben sich die Ergebnisse sehr stark verschlechtert.\todo{hm, also das keine einzige frage richtig ist, klingt für mich nach einem schwerwiegenderen Fehler als nur Überspezialisierung. Vielleicht ist irgendeine Option falsch eingestellt gewesen? das ist natürlich blöd so kurz vor der abgabe, das klingt wie etwas, das man langwierig überprüfen muss.}
