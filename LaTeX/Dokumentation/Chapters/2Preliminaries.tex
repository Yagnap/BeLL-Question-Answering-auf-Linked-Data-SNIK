...
Bei Beispiel zwei ist wichtig, dass \aurl{bb}{ChiefInformationOfficer} und \aurl{ciox}{ChiefInformationOfficer} nicht das gleiche Wissen zeigen.
...
Im Beispiel aus \cref{tab:rdftripleexample} ist also Frau Mustermann ein Element der Klasse \emph{Leiterin des Informationsmanagements}.
...
Relationen zweier Elemente werden als \emph{binäre Relationen} bezeichnet.
Relationen, die nicht binär sind, werden im Folgenden nicht benötigt.

In \ac{rdf} sind nämlich alle Relationen zweistellig, also Paare.
Die Beschreibung dieser Relation wird \emph{Property}, also Attribut oder Eigenschaft, genannt.
Properties nehmen die Rolle von Prädikaten an, so ist zum Beispiel \aurl{meta}{isResponsibleForEntityType} ein Property von der Ressource \aurl{bb}{ChiefInformationOfficer},
welche im Tripel die Rolle des Subjekts annimmt, mit dem das Objekt darstellenden Wert, \aurl{bb}{AnnualITBudget}.
Sie können auch eingeschränkt werden, indem man mit \ac{rdf} ihren \emph{Definitionsbereich} und \emph{Wertebereich} verändert.
...
es beschreibt demnach eine Eigenschaft von Ressourcen.
...
Es sind also immer nur die Klassen miteinander verbunden, die Individuen haben höchstens über Fremdschlüssel etwas miteinander zu tun, dann jedoch nur in einem vorher gesetzten Rahmen.
Bei Wissensbasen können Individuen in Relationen mit anderen Individuen sein.
Des Weiteren sind Wissensbasen auf zweistellige Relationen beschränkt,
wohingegen relationale Datenbanken eine deutlich höhere Anzahl an Spalten besitzen und somit eine deutlich höhere Stelligkeit der Relationen existiert.
...
Es sollen verschiedene open-source lizensierte Datenquellen in \ac{rdf} umgeformt und verbunden werden.
Dies kann zu Problemen im Sinne von fehlender Konsistenz der Daten führen \citep{semanticwebreview}.
\citet{linkeddatadesignissues} verfasste vier Regeln für das Veröffentlichen von Daten im Internet, um ein umfassendes Linked Data-System zu ermöglichen.
Diese sagen im Wesentlichen, dass klar mit \ac{http}-\acp{uri} benannte Ressourcen standardisiert durch \ac{rdf} und \ac{sparql} weitere nützliche Informationen und Links zu anderen \acp{uri} enthalten sollen.
...
Möglich ist auch, dass in dieser Abfrage eine weitere Variable genannt wird.
Dies kann zu mehreren Ergebnissen führen.
...
Dabei werden Prädikate hintereinander als \enquote{Weg} der Abfrage geschrieben.
Der Charakter \texttt{|} steht für einen alternativen Pfad, ähnlich dem logischen \emph{oder}.
Um die Suche weiter einzuschränken oder auszuweiten kann man das Schlüsselwort \texttt{FILTER} benutzen, nach diesem kann man zum Beispiel auch mehrere Objekte verwenden.
Bei beiden dieser Methoden kann man auch eine regulären Ausdrücken ähnliche Syntax nutzen.
Reguläre Ausdrücke sind Zeichenketten, mit deren Hilfe in Texten nach bestimmten Folgen, Mustern o.ä. gesucht werden kann.
...
Mit dem Zeichen \texttt{*} wird auch \aurl{bb}{ChiefInformationOfficer} selbst ausgewählt, da dieses Zeichen eine beliebig häufige Anzahl an Wiederholungen des Pfades zulässt.
Im Gegensatz zu \texttt{+} setzt \texttt{*} jedoch keine Mindestanzahl von einer Wiederholung voraus, sondern erlaubt auch Zahlen von null,
...
Ein Beispiel dafür wäre die Frage, wo eine Bank zu finden sei.
Die fragenstellende Person könnte entweder nach der Filiale einer Bankgesellschaft suchen oder eine Sitzgelegenheit meinen.
Das Programm, an das die Frage gestellt wird, muss nun mit wenig Kontext herausfinden,
welche Deutung impliziert wird
Das kann beispielsweise mit mehr Daten,
wie etwa dem vorherigen Suchverlauf (Hat sich die Person vorher nach Parks/Geldfilialen erkundigt?)
oder persönlichen Daten, wie dem Alter, möglich sein.
...
Wenn es sich zum Beispiel um eine Wissensbasis zu Parks handelt, ist es wahrscheinlicher, dass die Person nach Sitzgelegenheiten im Park sucht.

\begin{definition}[Semantisches Question Answering]
\ac{sqa}, oder auch \ac{kbqa}, ist die Beantwortung von Fragen, die \ac{rdf}-Daten in natürlicher Sprache gestellt werden.
...
In einer Pipeline werden verschiedene Komponenten eines Algorithmus oder Verfahren nacheinander ausgeführt.
...
Diese werden mit einem Wortcorpus vortrainiert und somit gut einsetzbar, und können beispielsweise ohne Veränderung bei Disambiguationsaufgaben verwendet werden.
...
\ac{bert} nutzt eine von der Cloze-Prozedur inspirierte Vorgehensweise namens \ac{mlm}, also \enquote{maskiertes Sprachmodell}.
...
Erst durch diese Vorgehensweise kann von den sehr aufgabenspezifischen Architekturen weg- und zu fine-tuning-basierten Ansätzen hingegangen werden.
...
Anfangs gab es meist nur eine versteckte Schicht, später wurden mehrere von diesen eingefügt und sogenannte \aclp{dnn} entstanden.
\begin{figure}%[htbp!]
\centering
