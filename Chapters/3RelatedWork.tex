...
QAnswer KG \citep{qanswer} ist ein an der Universtität Saint-Étienne entwickeltes Question Answering-System,
...
Es ist leider nicht open-source, steht online\footnote{Demo: \url{https://qanswer-frontend.univ-st-etienne.fr}\\Website: \url{https://www.qanswer.eu}} aber frei zur Verfügung.
...
Dazu werden, mithilfe von verschiedenen Parametern wie der Anzahl der in der Abfrage verwendeten mit einer Ressource assoziierten Wörter verglichen mit der Anzahl der Wörter in der Frage,
oder die Ähnlichkeit der Label, welche über \aurl{rdfs}{label} festgelegt werden, mit den Wörtern, und einer trainierten künstlichen Intelligenz die Kandidaten evaluiert.
...
Ein weiterer ausschlaggebender Punkt ist, dass dadurch Fragen in verschiedenen Fragen gestellt werden können, solange die Ontologie in diesen verfügbar ist.
...
Systeme wie gAnswer benötigten die Daten auch in einer speziellen Form, um sie verarbeiten zu können, hier reichen die \ac{rdf}-Daten aus.
Das hilft der Intuitivität/Benutzerfreundlichkeit des Systems weiter.
...
für das Label vom \aurl{bb}{ChiefInformationOfficer}, besitzen.
Bei \ac{snik} und den meisten anderen Ontologien ist das allerdings kein Problem, da sie es bereits besitzen.
Das zieht allerdings auch mit sich, dass andere Informationen, die in der Ontologie existieren, aber nicht im Label gespeichert sind, nicht mit betrachtet werden.
